...
@section('title', 'Create Department - AuthnAuth')
...
<main class="mx-auto max-w-2xl px-4 py-8 sm:px-6">
    ...
    <x-card>
        ...
        <form method="POST" action="{{ route('departments.store') }}" class="mt-6 flex flex-col gap-5">
            @csrf
            ...
            <x-field name="head_user_id" label="Department Head" as="select">
                ...
                @foreach ($users as $user)
                    <option value="{{ $user->id }}" @selected(old('head_user_id') == $user->id)>{{ $user->name }}</option>
                @endforeach
            </x-field>

            <x-button size="lg" class="w-full">Create Department</x-button>
        </form>
    </x-card>
