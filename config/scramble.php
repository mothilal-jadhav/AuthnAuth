...
return [
    ...
    'info' => [
         ...
         */
        'description' => "AuthnAuth's versioned REST API. Authenticate via ".
            'POST /api/v1/auth/login to receive a Sanctum bearer token, then send it as '.
            "'Authorization: Bearer <token>' on every other request. Every endpoint enforces the ".
            'same role/permission/ordinal-hierarchy rules as the web app — see UserPolicy in the '.
            'main codebase for the authoritative rules this API never duplicates or relaxes.',
    ],
     ...
     * ],
     */
    // Every protected endpoint uses `auth:sanctum` (see routes/api.php); this
    // makes Scramble mark those routes as requiring bearer auth in the
    // generated spec, and the public /auth/login route as security: [].
